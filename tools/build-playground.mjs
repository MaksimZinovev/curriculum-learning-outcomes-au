...
async function read(p, hint) {
  ...
  catch { throw new Error(`cannot read ${p}${hint ? ' — ' + hint : ''}`) }
}
function parseJson(text, p) {
  try { return JSON.parse(text) } catch (e) { throw new Error(`${p} is malformed JSON: ${e.message}`) }
}
function parseJsonl(text, p) {
  return text.trim().split('\n').filter(Boolean).map((l, i) => {
    try { return JSON.parse(l) } catch (e) { throw new Error(`${p} line ${i + 1} is malformed: ${e.message}`) }
  })
}

async function main() {
  const rows = parseJsonl(await read('data/outcomes.jsonl', 'run node sync.mjs first'), 'data/outcomes.jsonl')
    .map(r => ({
      code: r.code,
      title: r.title,
  ...
  const meta = parseJson(await read('data/sync-metadata.json', 'run node sync.mjs first'), 'data/sync-metadata.json')
  ...
  console.log(`[build] playground.html · ${rows.length} outcomes · ${(html.length / 1e6).toFixed(2)} MB`)
}
